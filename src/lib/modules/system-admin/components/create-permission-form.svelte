<script lang="ts">
	...
	let {
		...
		open = $bindable(false),
		defaultModuleId = ''
	}: { modules: ModuleNode[]; open?: boolean; defaultModuleId?: string } = $props();
	...
	let moduleId = $state('');
	let selectedEntity = $state<{ id: string; entity_name: string; description: string } | null>(null);
	let entitiesData = $state<any[]>([]);
	...
	$effect(() => {
		if (open) {
			if (defaultModuleId) moduleId = defaultModuleId;
		} else {
			name = '';
			slug = '';
			description = '';
	...
	async function fetchEntities(contextId: string) {
		...
		try {
			const { data, error } = await userCtx.client().GET('/api/system-admin/contextfind/{context_id}/entities', {
	...
	function getAllModulesFlat(nodes: ModuleNode[]): ModuleNode[] {
		let flat: ModuleNode[] = [];
		for (const node of nodes) {
			flat.push(node);
			if (node.children) {
	...
	const isEntityFieldDisabled = $derived(() => {
		if (!moduleId) return true;
		const allModulesFlat = getAllModulesFlat(modules);
		const module = allModulesFlat.find(m => m.id === Number(moduleId));
		return !module?.context_id || entitiesLoading;
	});
	...
	async function handleSubmit(e: Event) {
		...
		loading = true;
		try {
			const { error } = await userCtx.client().POST('/api/system-admin/permissions/', {
				body: {
					name,
					slug,
					description,
			...
			toast.success('Berhasil menambahkan hak akses');
			await queryClient.invalidateQueries({ queryKey: systemAdminKeys.permissionsAll() });
			open = false;
		} catch (err) {
			console.error(err);
			toast.error('gagal menambahkan hak akses');
		} finally {
			loading = false;
		}
	}
...
<Dialog.Root bind:open>
	<Dialog.Content class="sm:max-w-2xl sm:min-w-max">
		<Dialog.Header>
			<Dialog.Title>Tambah Izin/Hak akses baru</Dialog.Title>
			<Dialog.Description>tambahkan izin/hak akses baru untuk menu.</Dialog.Description>
		</Dialog.Header>
		<form onsubmit={handleSubmit} class="w-full">
			<div class="grid max-h-[60svh] w-full gap-4 overflow-y-auto px-2 py-4">
				...
				<div class="grid grid-cols-4 items-center gap-4">
					...
					<div class="col-span-3 w-full">
						<Popover.Root bind:open={entityPopoverOpen}>
							<Popover.Trigger bind:ref={entityTriggerRef} disabled={isEntityFieldDisabled()}>
								{#snippet child({ props })}
									<Button
										{...props}
										variant="outline"
										...
										disabled={isEntityFieldDisabled()}
									>
										<span class="inline-flex items-center gap-2">
											{#if entitiesLoading}
												Loading entities...
											{:else if isEntityFieldDisabled()}
												Pilih menu dengan context terlebih dahulu
											{:else}
												{selectedEntity?.entity_name || 'Pilih entity...'}
											{/if}
										</span>
									...
									<Command.List>
										...
										{#if entitiesData?.length}
											<Command.Group value="entities">
												{#each entitiesData as entity (entity.id)}
													<Command.Item
														value={entity.id}
														onSelect={() => {
													...
													>
														...
														<div class="flex flex-col">
															...
															{#if entity.description}
																<span class="text-sm text-muted-foreground">{entity.description}</span>
															{/if}
														</div>
													</Command.Item>
												{/each}
			...
			<Dialog.Footer>
				<Button type="submit" disabled={loading}>
					{loading ? 'Menambahkan...' : 'Tambah Akses'}
				</Button>
			</Dialog.Footer>
		</form>
	</Dialog.Content>
