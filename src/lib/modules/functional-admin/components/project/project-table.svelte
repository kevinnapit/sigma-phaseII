<script lang="ts">
	...
	import { useProjectListQuery } from '$lib/modules/functional-admin/index.svelte';
	...
	import ProjectConfirmDialog from './project-confirm-dialog.svelte';
	import Guard from '$lib/components/shared/guard.svelte';
	import { PROJECT_PERMISSIONS } from '../../constants/functional-admin-permissions';
	...
	let selectedForDelete = $state<ProjectItem | null>(null);
	...
	function openDelete(item: ProjectItem) {
	...
	const columns: ColumnDef<ProjectItem>[] = [
		...
		{
			accessorKey: 'project_type.value',
			id: 'project_type',
			header: 'Tipe Proyek',
			cell: ({ getValue }) => getValue() || '-'
		},
		{
			...
			cell: ({ row }) =>
				renderComponent(DataTableActions, {
					id: row.original.id,
					onUpdate: authCtx.hasPermission(PROJECT_PERMISSIONS.UPDATE)
						? () => openEdit(row.original)
						: undefined,

					onDelete: authCtx.hasPermission(PROJECT_PERMISSIONS.DELETE)
...
<ProjectForm bind:open={dialogOpen} mode={dialogMode} project={selectedProject} />

<DataTable.Root>
	<DataTable.Header title="Daftar Proyek" description="Kelola data proyek anda">
		{#snippet actions()}
			<Guard permissions={PROJECT_PERMISSIONS.CREATE}>
	...
	<DataTable.Toolbar>
		{#snippet start()}
			<DataTable.Search bind:value={search} debounce={100} placeholder="Cari Proyek" />
		{/snippet}
		{#snippet end()}
...
{#if selectedForDelete}
	<ProjectConfirmDialog bind:open={isDeleteDialogOpen} project={selectedForDelete} />
{/if}
