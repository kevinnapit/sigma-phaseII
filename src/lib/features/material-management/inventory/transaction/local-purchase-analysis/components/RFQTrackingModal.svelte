<script lang="ts">
	/**
	 * RFQTrackingModal - Tracking untuk Proses Pembelian Lokal (RFQ)
	 * Menampilkan 10 steps: RFQ → GRN Approved
	 */
	...
	import type { TrackingStatusItem } from '../types/local-purchase-analysis.types';
	...
	let {
		...
		rfqNumber: string;
		processType?: 'RFQ' | 'RO';
		trackingData?: TrackingStatusItem[];
	} = $props();
	...
	<Dialog.Content class="max-h-[80vh] max-w-3xl overflow-y-auto">
		<Dialog.Header>
			<Dialog.Title class="flex items-center gap-2">
				<Clock class="h-5 w-5" />
				Tracking {processType === 'RO' ? 'Repeat Order' : 'Proses Pembelian Lokal'}
			</Dialog.Title>
			<Dialog.Description>
				Kronologi status untuk dokumen <strong>{rfqNumber}</strong>
				{processType === 'RO' ? '(Repeat Order)' : '(RFQ)'}
			</Dialog.Description>
		</Dialog.Header>
