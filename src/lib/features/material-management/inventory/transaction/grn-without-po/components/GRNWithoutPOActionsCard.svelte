<script lang="ts">
	...
	import GRNWithoutPOConfirmSubmitModal from './GRNWithoutPOConfirmSubmitModal.svelte';
	import GRNWithoutPOSuccessModal from './GRNWithoutPOSuccessModal.svelte';
	...
	async function handleConfirmSubmit() {
		...
		try {
			...
			createdGRN = {
				number: result?.grn_number || 'GRN/WPO/2026/0001',
				id: result?.id || '1'
			};

			showSuccessModal = true;
		} catch (error) {
			console.error('Error submitting GRN Without PO:', error);
		}
	}
	...
	function handleBackToList() {
		...
		window.location.href =
			'/dashboard/material-management/inventory/transaction/grn-without-po';
	}
</script>
...
<!-- Modals -->
<GRNWithoutPOConfirmSubmitModal
