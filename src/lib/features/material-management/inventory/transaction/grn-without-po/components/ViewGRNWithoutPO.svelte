<script lang="ts">
	...
	import { useReadAllGRNWithoutPO } from '../hooks/useGRNWithoutPOQueries.svelte';
	import GRNWithoutPOActionsCells from './GRNWithoutPOActionsCells.svelte';
	import type { GRNWithoutPO } from '../types/grn-without-po.types';
	...
	// Query data
	const grnQuery = useReadAllGRNWithoutPO(() => ({
		page: currentPage,
		size: pageSize,
	...
	// Define columns
	const columns: ColumnDef<GRNWithoutPO>[] = [
		{
		...
		},
		{
			...
			header: 'Aksi',
			cell: ({ row }) =>
				renderComponent(GRNWithoutPOActionsCells, {
					grn: row.original,
	...
	function handleViewDetail(grn: GRNWithoutPO) {
		goto(
			`/dashboard/material-management/inventory/transaction/grn-without-po/${grn.id}`
		);
	}
	...
	function handleCreateNew() {
		goto('/dashboard/material-management/inventory/transaction/grn-without-po/create');
	}
</script>
...
<DataTable.Root>
	<DataTable.Header
		...
		description="Kelola penerimaan barang tanpa purchase order"
		class="md:mb-4"
	/>
