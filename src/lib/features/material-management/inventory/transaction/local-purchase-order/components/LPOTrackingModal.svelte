<script lang="ts">
	/**
	 * LPOTrackingModal - Tracking untuk Order Pembelian Lokal (LPO & LPOM)
	 * Menampilkan 7 steps: LPO Draft → GRN Approved
	 */
	...
	import type { LPOTrackingStatusItem } from '../types/local-purchase-order.types';
	...
	let {
		...
		lpoNumber: string;
		lpoType?: 'LPO' | 'LPOM';
		trackingData?: LPOTrackingStatusItem[];
	} = $props();
	...
	const itemsTracking = $derived(() => {
		const grouped = new Map<string, {
			...
			quantity: number;
			uom: string;
			tracking: LPOTrackingStatusItem[];
			current_step: number;
			total_steps: number;
	...
	function getItemNameByCode(itemCode: string): string {
		const itemMapping: Record<string, string> = {
			...
			'CLN-001': 'Sabun Cuci Tangan',
			'CLN-002': 'Sapu Lidi',
			'CLN-003': 'Pel Lantai',
			'CLN-004': 'Pembersih Lantai',
			'BEV-001': 'Kopi Bubuk',
			'BEV-002': 'Teh Celup',
			'TEST-001': 'Barang Test Perlu Izin'
		};
	...
	function extractItemQuantityUOM(remarks: string, itemCode: string): { quantity: number; uom: string } {
		const itemPatterns: Record<string, RegExp> = {
			'ATK-001': /Tissue.*?\((\d+)\s*(\w+)\)/i,
			'ATK-002': /Gula.*?\((\d+)\s*(\w+)\)/i,
			...
			'CLN-001': /Sabun.*?\((\d+)\s*(\w+)\)/i,
			'CLN-002': /Sapu.*?\((\d+)\s*(\w+)\)/i,
			'CLN-003': /Pel.*?\((\d+)\s*(\w+)\)/i,
			'CLN-004': /Pembersih.*?\((\d+)\s*(\w+)\)/i,
			'BEV-001': /Kopi.*?\((\d+)\s*(\w+)\)/i,
			'BEV-002': /Teh.*?\((\d+)\s*(\w+)\)/i,
			'TEST-001': /Test.*?\((\d+)\s*(\w+)\)/i
		};
		...
		const defaultQuantities: Record<string, { quantity: number; uom: string }> = {
			'ATK-001': { quantity: 30, uom: 'box' },
			'ATK-002': { quantity: 20, uom: 'kg' },
			...
			'CLN-001': { quantity: 20, uom: 'btl' },
			'CLN-002': { quantity: 20, uom: 'pcs' },
			'CLN-003': { quantity: 10, uom: 'pcs' },
			'CLN-004': { quantity: 15, uom: 'btl' },
			'BEV-001': { quantity: 10, uom: 'kg' },
			'BEV-002': { quantity: 50, uom: 'box' },
			'TEST-001': { quantity: 5, uom: 'pcs' }
		};
	...
	function getSmartTimelineSteps(tracking: LPOTrackingStatusItem[], currentStep: number, totalSteps: number) {
		const reversed = [...tracking].reverse();
	...
	<Dialog.Content class="max-h-[80vh] max-w-3xl overflow-y-auto">
		<Dialog.Header>
			<Dialog.Title class="flex items-center gap-2">
				<Clock class="h-5 w-5" />
				Tracking {lpoType === 'LPOM' ? 'Local Purchase Manual' : 'Order Pembelian Lokal'}
			</Dialog.Title>
			<Dialog.Description>
				Kronologi status untuk dokumen <strong>{lpoNumber}</strong>
				{lpoType === 'LPOM' ? '(Manual)' : '(LPO)'}
			</Dialog.Description>
		</Dialog.Header>
									...
									<div class="rounded-lg border-2 {isRejected ? 'border-destructive bg-destructive/10' : isCompleted ? 'border-green-500 bg-green-50' : 'border-primary bg-primary/10'} p-3">
										<div class="flex items-start gap-2">
											...
											<div class="flex-1">
												...
												{/if}
												{#if isRejected && currentStatus.remarks && !currentStatus.remarks.startsWith('Items:')}
										...
										{#each displaySteps as track, index}
											...
											<div class="relative flex gap-3 {isCurrent ? 'opacity-50' : ''}">
												...
												<div class="flex-1 pb-1">
													...
													{/if}
													{#if isRejected && track.remarks && !track.remarks.startsWith('Items:')}
