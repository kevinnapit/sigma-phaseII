<script lang="ts">
	...
	import { useSubmitGRNReconditionedStock } from '../hooks/useGRNReconditionedStockMutations.svelte';
	...
	// Submit mutation
	const submitMutation = useSubmitGRNReconditionedStock();
	...
	function handleViewDetail() {
		goto(
			`/dashboard/material-management/inventory/transaction/grn-reconditioned-stock/${grnId}`
		);
	}
</script>
			...
			<div class="flex flex-col items-center space-y-4 text-center">
				...
				<div class="space-y-2">
					<Dialog.Title class="text-xl font-semibold">
						GRN Re-Conditioned Stock Berhasil Dibuat
					</Dialog.Title>
					<Dialog.Description class="text-sm text-muted-foreground">
						Penerimaan barang re-conditioned stock Anda telah berhasil dibuat dengan nomor:
					</Dialog.Description>
				</div>
			</div>
		</Dialog.Header>
			...
			<div class="rounded-lg border bg-muted/50 p-4">
				<div class="flex items-center justify-between">
					<div class="flex flex-col">
						<span class="text-xs text-muted-foreground">Nomor GRN Re-Conditioned Stock</span>
