<script lang="ts">
	...
	import {
		useApproveGRNReconditionedStock,
		useRejectGRNReconditionedStock
	} from '$lib/features/material-management/inventory/transaction/grn-reconditioned-stock/hooks/useGRNReconditionedStockMutations.svelte';
	...
	// ── Mutations ────────────────────────────────────────────────────────────────
	const approveMutation = useApproveGRNReconditionedStock();
	const rejectMutation = useRejectGRNReconditionedStock();
	...
	function handleBack() {
		...
		goto(
			`/dashboard/material-management/inventory/transaction/grn-reconditioned-stock${params ? `?${params}` : ''}`
		);
	}
...
<div class="space-y-6">
	...
	<div class="items-center justify-between sm:flex">
		<PageHeader
			title="Detail Penerimaan Barang Re-Conditioned Stock"
			description="Informasi lengkap penerimaan barang re-conditioned stock"
		/>
	...
	<div class="grid gap-6 lg:grid-cols-3">
		...
		<div class="space-y-6 lg:col-span-2">
			...
			<Card.Root>
				...
				<Card.Content>
					<!-- Desktop table (15 columns, no Offered Qty) -->
						...
						<table class="w-full text-sm">
							...
							<tfoot>
								<tr class="border-t font-semibold">
									<td colspan="14" class="p-2 text-right">Gross Amount</td>
	...
	<AlertDialog.Content>
		<AlertDialog.Header>
			...
			<AlertDialog.Description>
				Apakah Anda yakin ingin menyetujui penerimaan barang re-conditioned stock ini?
			</AlertDialog.Description>
		</AlertDialog.Header>
...
<AlertDialog.Root bind:open={showRejectDialog}>
	<AlertDialog.Content>
		<AlertDialog.Header>
			...
			<AlertDialog.Description>
				Anda akan menolak penerimaan barang re-conditioned stock ini. Silakan berikan alasan
				penolakan.
			</AlertDialog.Description>
		</AlertDialog.Header>
		<div class="space-y-2 py-4">
